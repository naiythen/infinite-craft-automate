...
def send_combination_request(word1, word2, current_driver):
    ...
    try:
        ...
        if int(response_data['status']) == 200:
            ...
            print(f"Request failed with status {response_data['status']} for combination {word1} + {word2}.")
            input(f"Request failed for {word1} + {word2}. Status: {response_data['status']}. Press Enter to continue...")
            return None
    except Exception as e:
        print(f"Exception during request for {word1} + {word2}: {e}")
...
def process_result(api_result):
    ...
    try:
        ...
        print(f"Error processing result for '{result_word_processed}' from {word1} + {word2}: {e}")
        input("Press Enter to continue...")
        return False
...
def run_iterations_sequentially():
    ...
    for i in range(iterations):
        ...
        if not current_words_pool_for_iteration or len(current_words_pool_for_iteration) < 2 :
            ...
            if i == 0 and len(current_words_pool_for_iteration) < 2 : # If first iter and still no words
                 print("Cannot proceed without at least two elements. Exiting iterations.")
                 return # Exit iterations if no words available for first try
            elif len(current_words_pool_for_iteration) < 2:
                 print("Skipping this iteration due to lack of elements.")
                 continue # Skip to next iteration if not the first

        while attempt < max_attempts_per_iteration and not processed_new_in_this_iteration:
            ...
            if pair_tuple not in tried_combinations and pair_tuple not in existing_recipes:
                ...
                print(f"Attempting combination: {word1} + {word2}")
                ...
                if api_response:
                    ...
                    print(f"Combination {word1} + {word2} did not yield a usable API response or was skipped.")
            else:
                # This pair was already tried or recipe exists.
                # To prevent infinite loops if all combinations are exhausted quickly, increment attempt.
                attempt +=1 # Count this as an attempt to find a *new* one
                if attempt >= max_attempts_per_iteration / 2 and len(tried_combinations) >= (len(current_words_pool_for_iteration) * (len(current_words_pool_for_iteration)-1))/2 : # Heuristic
                    print("Many combinations tried or existing. Refreshing word pool early.")
                    current_words_pool_for_iteration = list(set(get_initial_words(load_json(json_file_path))))


            if not processed_new_in_this_iteration : # only increment if we didn't find one. if we did, loop breaks.
                 attempt += 1
            
        if not processed_new_in_this_iteration:
            print(f"Could not find and process a new, untried combination after {attempt} attempts for iteration {i+1}.")
...
try:
    ...
    print(f"An unexpected error occurred during the main iteration process: {e}")
    input("Press Enter to acknowledge and exit...")
finally:
    if driver:
        try:
